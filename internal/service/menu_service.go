...
func (s *menuService) GetMenuItemByID(ctx context.Context, id int) (models.MenuItems, error) {
	if id <= 0 {
		return models.MenuItems{}, fmt.Errorf("invalid menu item id")
	}
...
func (s *menuService) CreateMenuItem(ctx context.Context, item models.MenuItems) (int, error) {
	if item.Name == "" {
		return 0, fmt.Errorf("invalid menu item name")
	}
	if item.Price <= 0 {
		return 0, fmt.Errorf("invalid menu item price")
	}
...
func (s *menuService) UpdateMenuItem(ctx context.Context, id int, item models.MenuItems) error {
	...
	if item.Name == "" {
		return fmt.Errorf("invalid menu item name")
	}
	if item.Price <= 0 {
		return fmt.Errorf("invalid menu item price")
	}
